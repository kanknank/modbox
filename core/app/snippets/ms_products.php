...
if ($cache && !in_array($modx->resource->alias, ['favorites'])) {
...
foreach ($props as $key => $val) {
    ...
    if (preg_match('/filter_/i', $key)) {
        ...
        } else if ($key == 'action' && isset($val[0]) && in_array($val[0], ['new', 'favorite', 'popular'])) {
            $where[] = "Data.{$val[0]} = 1";
        } else {
...
if ($modx->resource->alias == 'novinki' || $modx->resource->alias == 'catalog-new') {
    $parents = null;
    $where[] = "Data.new = 1";
}

if ($modx->resource->alias == 'favorites') {
    $parents = null;
    $ids = json_decode($_COOKIE['favorites']) ?? [];
    $ids = implode(',', $ids);
    $where[] = "msProduct.id IN ($ids)";
}
